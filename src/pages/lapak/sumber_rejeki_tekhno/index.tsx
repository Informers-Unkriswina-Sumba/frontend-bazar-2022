...
const Kenangan: NextPage = () => {
  ...
  const getListProduct = async () => {
    ...
    const res = await ApiGetListProdukByLapak(LIST_LAPAK_DATA.KELOMPOK_8.id);
    if (res.status === 200) {
  ...
  const getLapak = async () => {
    ...
    const res = await ApiGetLapakDetailBySlugName(
      LIST_LAPAK_DATA.KELOMPOK_8.slugName
    );
    if (res.status === 200) {
  ...
  return (
    <LayoutMainApp>
      <Head>
        <title>
          {APP_TITLE} | Lapak {LIST_LAPAK_DATA.KELOMPOK_8.namaLapak}
        </title>
      ...
      <Box>
        {loadingGetLapak || !lapak ? (
          ...
          <ProfileLapakItem
            ...
            linkProfile={`/lapak/${LIST_LAPAK_DATA.KELOMPOK_8.slugName}/profile`}
          />
        )}
      </Box>
      <Box px='15' mt='10'>
